...
export const CancelRideModal: React.FC<CancelRideModalProps> = ({
    ...
    const title = isRide ? "ביטול רישום לנסיעה" : "ביטול רישום לפריט";
    const message = isRide
        ? (isTwinAvailable ? 'שמנו לב שאתה רשום גם לנסיעה בכיוון השני.' : 'האם אתה בטוח שברצונך לבטל את הרישום לנסיעה?')
        : 'האם אתה בטוח שברצונך לבטל את הרישום לפריט זה?';
    const confirmBtnText = isRide ? "כן, בטל את הנסיעה" : "כן, בטל את הרישום";
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <FocusTrap active={isOpen}>
                <div role="dialog" aria-modal="true" className="bg-white rounded-xl shadow-xl max-w-sm w-full overflow-hidden flex flex-col animate-fadeIn">
                    ...
                    <div className={`flex items-center justify-between p-6 border-b flex-none rounded-t-xl text-white ${isRide ? 'bg-rides-dark' : 'bg-gray-700'}`}>
