      ...
      {showOffers && (
        ...
        >
          <div className="relative z-10 flex flex-col items-center">
            ...
            />
            <h3 className="text-base font-bold text-neutral-800">{t('categorySelector.rideOffers.title')}</h3>
            <p className="text-[10px] text-neutral-500 mt-1 font-medium bg-rides-bg text-rides-primary px-2 py-0.5 rounded-full">
              {t('categorySelector.rideOffers.seatsAvailable', { count: offerProgress.total - offerProgress.assigned })}
            </p>
          </div>
      ...
      {showRequests && (
        ...
        >
          <div className="relative z-10 flex flex-col items-center">
            ...
            />
            <h3 className="text-base font-bold text-neutral-800">{t('categorySelector.rideRequests.title')}</h3>
            <p className="text-[10px] text-neutral-500 mt-1 font-medium bg-rides-bg text-rides-primary px-2 py-0.5 rounded-full">
              {t('categorySelector.rideRequests.passengersWaiting', { count: requestProgress.total - requestProgress.assigned })}
            </p>
          </div>
                ...
                <div className="w-full">
                  <p className="text-center text-neutral-500 text-sm mb-4">
                    {category.id === 'ride_offers' || category.id === 'trempim'
                      ? t('categorySelector.stats.seatsFree', { count: progress.total - progress.assigned })
                      : category.id === 'ride_requests'
                        ? t('categorySelector.rideRequests.passengersWaiting', { count: progress.total - progress.assigned })
                        : t('categorySelector.stats.assigned', { assigned: progress.assigned, total: progress.total })
                    }
                  </p>
